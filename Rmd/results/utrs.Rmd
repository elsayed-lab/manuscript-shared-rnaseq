...
for (stage in names(top_genes)) {
    ...
    x$de_status <- 'Non-DE'
    x$de_status[x$adj.P.Val < 0.05 & x$logFC > 0] <- sprintf('Up-reg (n=%d)', num_up)
    x$de_status[x$adj.P.Val < 0.05 & x$logFC < 0] <- sprintf('Down-reg (n=%d)', num_down)
...
```{r utr5_length_by_de_status, results='asis', fig.height=810/96}
library('grid')
...
```{r utr3_length_by_de_status, results='asis', fig.height=810/96}
