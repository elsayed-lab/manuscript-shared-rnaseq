...
for (stage in names(top_genes)) {
    ...
    # bar plots for contrasts with differing Tx and UTR lengths
    ...
    if (abs(xx$median_tx_len[UPREG_IDX] - xx$median_tx_len[DOWNREG_IDX]) > 10) {    
        tx_plts[[stage]] <- ggplot(xx, aes(de_status, y=median_tx_len)) +
            geom_bar(stat = "identity") +
            labs(title=sprintf("%s hrs", gsub('_', ' ', stage))) +
            theme(axis.title.x=element_blank(),
                axis.title.y=element_blank(),
...
```{r tx_length_by_de_status, results='asis'}
if (length(tx_plts) > 0) {
...
ggplot(dat, aes(condition, log_cpm, group=gene_id, color=txlen)) +
    geom_line()
