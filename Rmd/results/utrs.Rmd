...
for (stage in names(top_genes)) {
    ...
    utr_info <- result %>% select(gene_id, utr5_len, utr3_len)
...
for (stage in names(top_genes)) {
    ...
    x$de_status <- 'Non-DE'
    x$de_status[x$adj.P.Val < 0.05 & x$logFC > 0] <- sprintf('Up-reg\n(%d genes)', num_up)
    x$de_status[x$adj.P.Val < 0.05 & x$logFC < 0] <- sprintf('Down-reg\n(%d genes)', num_down)

    cat(sprintf('\n#### %s (%d up / %d down)\n\n', stage, num_up, num_down))

    utr_info <- result %>% select(gene_id, 
                                  utr5_gc_percent, utr5_ct_percent,
    ...
    # bar plots
    if (abs(xx$median_5utr_gc_richness[3] - xx$median_5utr_gc_richness[1]) > 0.035) {    
        utr5_plts[[stage]] <- ggplot(xx, aes(de_status, y=median_5utr_gc_richness)) +
            geom_bar(stat="identity") +
            labs(title=sprintf("%s hrs", gsub('_', ' ', stage)))
    }

    if (abs(xx$median_3utr_gc_richness[3] - xx$median_3utr_gc_richness[1]) > 0.035) {    
        utr3_plts[[stage]] <- ggplot(xx, aes(de_status, y=median_3utr_gc_richness)) +
            geom_bar(stat="identity") +
            labs(title=sprintf("%s hrs", gsub('_', ' ', stage)))
    }
}
...
```{r utr5_gc_richness_by_de_status, results='asis'}
if (length(utr5_plts) > 0) {
    grid.arrange(grobs=utr5_plts, ncol=3,
                bottom='Differential Expression Status',
                left="Median 5' UTR GC-richness'")
}
```

```{r utr3_gc_richness_by_de_status, results='asis'}
if (length(utr3_plts) > 0) {
    grid.arrange(grobs=utr3_plts, ncol=3,
                bottom='Differential Expression Status',
...
```{r gene_expression_grouped_by_utr_length, results='asis'}
...
ggplot(dat, aes(condition, log_cpm, group=gene_id, color=utr_length)) +
    geom_line()

# average expression of top N genes with longest/shortest UTR's
dat2 <- dat %>% 
    group_by(utr_length, condition) %>% 
    summarise(expr=mean(log_cpm))
