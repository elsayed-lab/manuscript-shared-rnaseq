...
for (stage in names(top_genes)) {
    x <- top_genes[[stage]]
    x$de_status <- 'Non-DE'
    x$de_status[x$adj.P.Val < 0.05 & x$logFC > 0] <- 'Up-regulated'
    x$de_status[x$adj.P.Val < 0.05 & x$logFC < 0] <- 'Down-regulated'

    cat(sprintf('\n#### %s (%d up / %d down)\n\n', stage,
                sum(x$de_status == 'Up-regulated'),
                sum(x$de_status == 'Down-regulated')))
