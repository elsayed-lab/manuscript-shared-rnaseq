...
TODO: Add explanation of filtering approach.
...
# so we will also set a lower limit equal to 5% of the samples
min_replicates <- max(min_replicates, round(length(condition) / 20))

de_counts$raw <- filter_lowcount_genes(de_counts$raw, 
                                       threshold=CONFIG$low_count_threshold,
                                       min_samples=min_replicates, verbose=TRUE)
```

```{r include=CONFIG$verbose, echo=CONFIG$verbose}
print("Removing low count genes:")
print(head(rownames(assay_data)[!rownames(assay_data) %in% rownames(de_counts$raw)]))
```
...
# TODO: Extend human/mouse filtering to include tRNAs
# library('FDb.UCSC.tRNAs')
# names(features(FDb.Hsapiens.UCSC.hg19.tRNAs))
# Returns names from UCSC table, but not corresponding ENSEMBL ids?

# Filter by gene id or type
# Note that when filtering by type for T. cruzi, SRPs and SLRNAs are not
# included
